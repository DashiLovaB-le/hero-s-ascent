...
CREATE TYPE public.attribute_type AS ENUM ('forca','disciplina','sabedoria','espirito','testosterona','prosperidade','conhecimento','lideranca');
CREATE TYPE public.goal_category AS ENUM ('corpo','mente','espirito','prosperidade','relacionamentos','proposito');
...
CREATE POLICY "Atualizar próprio perfil" ON public.profiles FOR UPDATE TO authenticated USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
CREATE POLICY "Inserir próprio perfil" ON public.profiles FOR INSERT TO authenticated WITH CHECK (auth.uid() = id);
...
CREATE POLICY "Ver próprios atributos" ON public.attributes FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Atualizar próprios atributos" ON public.attributes FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Todos veem níveis" ON public.levels FOR SELECT USING (true);
...
CREATE POLICY "Todos veem capítulos" ON public.chapters FOR SELECT USING (true);
...
CREATE POLICY "Todos veem conquistas" ON public.achievements FOR SELECT USING (true);
...
CREATE POLICY "Ver próprias conquistas" ON public.user_achievements FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Registrar próprias conquistas" ON public.user_achievements FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Gerenciar próprias metas" ON public.goals FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Gerenciar próprios hábitos" ON public.habits FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Ver próprias conclusões" ON public.habit_completions FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Registrar próprias conclusões" ON public.habit_completions FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Desfazer próprias conclusões" ON public.habit_completions FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY "Ver próprio histórico" ON public.activity_history FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Inserir próprio histórico" ON public.activity_history FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE INDEX idx_activity_history_user_date ON public.activity_history (user_id, created_at DESC);
...
-- =========== TRIGGER: novo usuário cria profile + attributes + role user ===========
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  INSERT INTO public.profiles (id, nome)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'nome', NEW.raw_user_meta_data->>'name', split_part(NEW.email,'@',1), 'Herói'));
